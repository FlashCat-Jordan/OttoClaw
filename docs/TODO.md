# MiaomiaoClaw vs Nanobot — Feature Gap Tracker

> Comparing against `nanobot/` reference implementation. Tracks features MiaomiaoClaw has not yet aligned with.
...
- **MiaomiaoClaw**: `memory_write_long_term` and `memory_append_today` exist but are only called from CLI; agent loop never writes memory
...
- **MiaomiaoClaw**: No authentication; anyone can message the bot and consume API credits
...
- **MiaomiaoClaw**: Uses `parse_mode: Markdown` directly; special characters can cause send failures (has fallback to plain text)
...
- **MiaomiaoClaw**: Not handled; /start is sent to Claude as a regular message
...
- **MiaomiaoClaw**: Only processes `message.text`, ignores all media messages
...
- **MiaomiaoClaw**: Only loads `SOUL.md` and `USER.md`
...
- **MiaomiaoClaw**: `context_builder.c` only reads last 3 days
...
- **MiaomiaoClaw**: `mimi_msg_t` only has channel + chat_id + content
...
- **MiaomiaoClaw**: Hardcoded if-else dispatch
...
- **MiaomiaoClaw**: Hardcoded to Anthropic Messages API
...
- **MiaomiaoClaw**: Basic JSON protocol, lacks streaming token push
...
- **MiaomiaoClaw**: Hardcoded in app_main()
...
- **MiaomiaoClaw**: Only Telegram + WebSocket
...
- **MiaomiaoClaw**: JSONL only stores role/content/ts, no metadata header
