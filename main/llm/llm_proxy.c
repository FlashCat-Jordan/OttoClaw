...
{
    if (s_provider[0] == '\0') return false;
...
static bool provider_is_groq(void)
{
    if (s_provider[0] == '\0') return false;
    return strcmp(s_provider, "groq") == 0;
}
...
static bool provider_is_vllm(void)
{
    if (s_provider[0] == '\0') return false;
    return strcmp(s_provider, "vllm") == 0;
}

static bool provider_is_openai_compatible(void)
{
    return provider_is_openai() || provider_is_qwen() || 
           provider_is_deepseek() || provider_is_groq() || 
           provider_is_zhipu() || provider_is_vllm();
}

static const char *llm_api_url(void)
{
    /* If custom base URL is set, use it (but not for Qwen) */
    if (s_api_base_url[0] != '\0' && !provider_is_qwen()) {
...
{
    /* Extract host from custom base URL if set */
    if (s_api_base_url[0] != '\0') {
        /* For Aliyun Bailian: dashscope.aliyuncs.com */
...
{
    /* Extract path from custom base URL if set */
    if (s_api_base_url[0] != '\0') {
        /* For Aliyun Bailian: /apps/anthropic/v1/messages */
...
static esp_err_t llm_http_direct(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    /* Authentication: use Bearer token for OpenAI-compatible providers or custom base URL */
    if (s_api_base_url[0] != '\0' || provider_is_openai_compatible()) {
        if (s_api_key[0]) {
            char auth[192];
        ...
        /* Gemini API uses x-goog-api-key */
        if (s_api_key[0]) {
            char auth[256];
            snprintf(auth, sizeof(auth), "x-goog-api-key: %s", s_api_key);
